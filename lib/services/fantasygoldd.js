...
var FantasyGoldRPC = require('fantasygoldd-rpc');
...
/**
 * Provides a friendly event driven API to fantasygoldd in Node.js. Manages starting and
 * stopping fantasygoldd as a child process for application support, as well as connecting
 * to multiple fantasygoldd processes for server infrastructure. Results are cached in an
 * LRU cache for improved performance and methods added for common queries.
 *
 * @param {Object} options
 * @param {Node} options.node - A reference to the node
 */
function FantasyGold(options) {
  if (!(this instanceof FantasyGold)) {
    return new FantasyGold(options);
  }
...
util.inherits(FantasyGold, Service);

FantasyGold.dependencies = [];

FantasyGold.DEFAULT_MAX_TXIDS = 1000;
FantasyGold.DEFAULT_MAX_HISTORY = 50;
FantasyGold.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
FantasyGold.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
FantasyGold.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
FantasyGold.DEFAULT_SPAWN_RESTART_TIME = 5000;
FantasyGold.DEFAULT_SPAWN_STOP_TIME = 10000;
FantasyGold.DEFAULT_TRY_ALL_INTERVAL = 1000;
FantasyGold.DEFAULT_REINDEX_INTERVAL = 10000;
FantasyGold.DEFAULT_START_RETRY_INTERVAL = 5000;
FantasyGold.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
FantasyGold.DEFAULT_TRANSACTION_CONCURRENCY = 5;
FantasyGold.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
  ...
  uacomment: 'fantasygoldcore'
};

FantasyGold.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || FantasyGold.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || FantasyGold.DEFAULT_MAX_HISTORY;
  this.maxAddressesQuery = options.maxAddressesQuery || FantasyGold.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || FantasyGold.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || FantasyGold.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || FantasyGold.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || FantasyGold.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || FantasyGold.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || FantasyGold.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || FantasyGold.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

FantasyGold.prototype._initCaches = function() {
...
FantasyGold.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
FantasyGold.prototype.getAPIMethods = function() {
  var methods = [
 ...
 */
FantasyGold.prototype.getPublishEvents = function() {
  return [
    {
...
FantasyGold.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'fantasygoldd/' + name, 'total:', this.subscriptions[name].length);
};

FantasyGold.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'fantasygoldd/' + name, 'total:', this.subscriptions[name].length);
};


FantasyGold.prototype.subscribeBalance = function(emitter, addresses) {
    var self = this;
    ...
    log.info(emitter.remoteAddress, 'subscribe:', 'fantasygoldd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

FantasyGold.prototype.unsubscribeBalance = function(emitter, addresses) {
    ...
    log.info(emitter.remoteAddress, 'unsubscribe:', 'fantasygoldd/addressbalance', 'total:', _.size(this.subscriptions.balance));
};

FantasyGold.prototype.unsubscribeBalanceAll = function(emitter) {
...
FantasyGold.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'fantasygoldd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

FantasyGold.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'fantasygoldd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
FantasyGold.prototype.unsubscribeAddressAll = function(emitter) {
...
FantasyGold.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = FantasyGold.DEFAULT_CONFIG_SETTINGS;
...
FantasyGold.prototype._parseBitcoinConf = function(configPath) {
  var options = {};
...
FantasyGold.prototype._expandRelativeDatadir = function() {
...
FantasyGold.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in fantasygoldd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in fantasygoldd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in fantasygoldd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './fantasygold.conf');

  log.info('Using fantasygold config file:', configPath);
...
FantasyGold.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of fantasygoldcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of fantasygoldcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of fantasygoldcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to fantasygoldd from bitcore. ' +
      'Please add "server=1" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubrawtx,
    '"zmqpubrawtx" option is required to get event updates from fantasygoldd. ' +
      'Please add "zmqpubrawtx=tcp://127.0.0.1:<port>" to your configuration and restart'
  );

  $.checkState(
    spawnConfig.zmqpubhashblock,
    '"zmqpubhashblock" option is required to get event updates from fantasygoldd. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that fantasygoldd is undergoing a reindex. ' +
             ...
             'of fantasygoldcore-node services will start.');
    node._reindex = true;
  }
};

FantasyGold.prototype._resetCaches = function() {
...
FantasyGold.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
FantasyGold.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

FantasyGold.prototype._initChain = function(callback) {
  var self = this;
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          self.emit('ready');
          log.info('FantasyGold Daemon Ready');
          callback();
        });
      });
...
FantasyGold.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 57810
  };
...
FantasyGold.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/fantasygold.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/fantasygold.conf';
    }
  }
  return networkPath;
};

FantasyGold.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === bitcore.Networks.testnet) {
...
FantasyGold.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('fantasygoldd/hashblock', message.toString('hex'));
    }
  }

};

FantasyGold.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
FantasyGold.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('FantasyGold Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
FantasyGold.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
FantasyGold.prototype._notifyBalanceSubscribers = function(txid, transaction) {
...
FantasyGold.prototype._notifyBalanceSubscriber = function (address, txid) {
    var self = this;
    self.getAddressSummary(address, {noTxList: false}, function(err, data) {
        ...
        if (emitters) {
            for(var j = 0; j < emitters.length; j++) {
                emitters[j].emit('fantasygoldd/addressbalance', {
                    address: address,
                    txid: txid,
...
FantasyGold.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  var addresses = this._getAddressesFromTransaction(transaction);
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('fantasygoldd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
FantasyGold.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('fantasygoldd/rawtransaction', message.toString('hex'));
    }
...
FantasyGold.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || FantasyGold.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

FantasyGold.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
FantasyGold.prototype._initZmqSubSocket = function(node, zmqUrl) {
...
FantasyGold.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('FantasyGold Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
    ...
    }, node._reindexWait || FantasyGold.DEFAULT_REINDEX_INTERVAL);
  } else {
    callback();
  }
};

FantasyGold.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
FantasyGold.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/fantasygoldd.pid';
...
FantasyGold.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting fantasygold process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('FantasyGold process unexpectedly exited with code:', code);
        log.warn('Restarting fantasygold child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('FantasyGold process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      node.client = new FantasyGoldRPC({
        protocol: 'http',
        host: '127.0.0.1',
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn fantasygoldd.'));
      }
...
FantasyGold.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to fantasygoldd.'));
    }
 ...
 */
FantasyGold.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('FantasyGold configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.syncPercentage = function(callback) {
  var self = this;
...
FantasyGold.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, fantasygoldcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
FantasyGold.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
...
FantasyGold.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
FantasyGold.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
FantasyGold.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in fantasygoldcore is most recent to less recent
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getAddressTxids = function(addressArg, options, callback) {
  /* jshint maxstatements: 20 */
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
...
FantasyGold.prototype._getConfirmationsDetail = function(transaction) {
...
FantasyGold.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
FantasyGold.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
FantasyGold.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A fantasygoldcoin transaction id
 * @param {Function} callback
 */
FantasyGold.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
FantasyGold.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
...
FantasyGold.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getAddressesMempoolBalance = function(addressArg, options, callback) {
    var self = this;
    var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
...
FantasyGold.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
FantasyGold.prototype.getJsonBlock = function(blockArg, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getBlockHashesByTimestamp = function(high, low, options, callback) {
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.estimateFee = function(blocks, callback) {
  var self = this;
...
FantasyGold.prototype.estimateSmartFee = function (blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getJsonRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a FantasyGoldcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
FantasyGold.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getInfo = function(callback) {
...
FantasyGold.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('fantasygoldd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('fantasygoldd process did not exit'));
      }
 ...
 * @param {Array} addresses - A array of fantasygold addresses to filter
 * @param {Function} callback
 */
FantasyGold.prototype.listUnspent = function(minConf, maxConf, addresses, callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getNewAddress = function(callback) {
    var self = this;
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.callContract = function(address, hash, options, callback) {
    var self = this,
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getAccountInfo = function(address, callback) {
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getSubsidy = function(height, callback) {
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getTransactionReceipt = function(trxHash, callback) {
 ...
 * @param {Function} callback
 */
FantasyGold.prototype.getDgpInfo = function(callback) {
 ...
 * @return {*}
 */
FantasyGold.prototype.getMiningInfo = function(callback) {
    var self = this,
 ...
 * @return {*}
 */
FantasyGold.prototype.getStakingInfo = function(callback) {
    var self = this,
