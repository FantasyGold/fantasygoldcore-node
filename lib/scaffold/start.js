...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for fantasygoldd is necessary for this upgrade with the "reindex=1" bitcoin.conf option. \n' +
                 'There are changes necessary in both bitcoin.conf and fantasygoldcore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed fantasygoldcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './fantasygoldcore-node.json');
  ...
  node.on('ready', function() {
    log.info('FantasyGoldcore Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in fantasygoldcore-node services directory
    ...
    if (servicePackage.fantasygoldcoreNode) {
      serviceModule = service.name + '/' + servicePackage.fantasygoldcoreNode;
    }
