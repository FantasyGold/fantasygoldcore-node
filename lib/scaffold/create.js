...
var BASE_PACKAGE = {
  ...
  dependencies: {
    ...
    'fantasygoldcore-lib': 'https://github.com/fantasygold/fantasygoldcore-lib.git#master',
    'fantasygoldcore-node': 'https://github.com/fantasygold/fantasygoldcore-node.git#master'
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/fantasygoldcore-node.json', configJSON);
      if (!isGlobal) {
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the fantasygoldcore-node directory and configuration
